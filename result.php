...
if (isset($_GET["prod_search"])) {
    $searchValue = $_GET["prod_search"];
    $results = $product->searchProduct($searchValue, $page, $perPage);
    $endPage = ceil($product->amountResultSearch($searchValue) / $perPage);
}
...
<body>
    ...
    <div id="content">
        <div id="content-header">
            ...
            <h6>Result: found <?= $product->amountResultSearch($searchValue) ?> item(s) with keyword "<?= isset($_GET["prod_search"]) ? $_GET["prod_search"] : "" ?>"</h6>
        </div>
                            ...
                            <div class="row" style="margin-left: 18px;">
                                <ul class="pagination">
                                    ...
                                    if ($endPage > 1) {
                                        for ($i = 0; $i < $endPage; $i++) {
                                    ?>
                                            <li class="<?= isset($_GET["page"]) && $_GET["page"] == ($i + 1) ? "active" : "" ?>"><a href="./result.php?prod_search=<?= $_GET['prod_search'] ?>&page=<?= $i + 1 ?>"><?= $i + 1 ?></a></li>
                                    <?php
                                        }
                                    }
                                    ?>
